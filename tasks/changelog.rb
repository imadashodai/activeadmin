class Changelog
  ...
  def pre_pull_request_references
    content.take_while { |line| !pull_reference?(line) }
  end

  def post_pull_request_references
    content.drop_while { |line| !pull_reference?(line) }.drop_while { |line| pull_reference?(line) }
  end

  def pull_request_references
    content.drop_while { |line| !pull_reference?(line) }.take_while { |line| pull_reference?(line) }
  end
